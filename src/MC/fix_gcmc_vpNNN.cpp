...
FixGCMCVp::FixGCMCVp(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
  ...
  size_vector =
      9;    // added by Jibao according to Matias' 2012 lammps version, to output energyout
  global_freq = 1;
  extvector = 0;
  restart_global = 1;
  time_depend = 1;
  ...
  if (regionflag) {
    ...
    double max_region_volume =
        (region_xhi - region_xlo) * (region_yhi - region_ylo) * (region_zhi - region_zlo);
  ...
  if (mode == MOLECULE) {
    if (onemols[imol]->xflag == 0) error->all(FLERR, "Fix gcmc molecule must have coordinates");
    if (onemols[imol]->typeflag == 0) error->all(FLERR, "Fix gcmc molecule must have atom types");
    if (ngcmc_type != 0) error->all(FLERR, "Atom type must be zero in fix gcmc mol command");
  ...
  if (shakeflag && mode == ATOM) error->all(FLERR, "Cannot use fix gcmc shake and not molecule");
  ...
  if (mode == ATOM)
    ...
    natoms_per_molecule = onemols[imol]->natoms;
...
void FixGCMCVp::options(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "mol") == 0) {
      ...
      if (imol == -1) error->all(FLERR, "Molecule template ID for fix gcmc does not exist");
      ...
      if (iregion == -1) error->all(FLERR, "Region ID for fix gcmc does not exist");
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg], "pair") == 0) {    // Matias
      if (iarg + 2 > narg) error->all(FLERR, "Illegal fix GCMC command");
...
FixGCMCVp::~FixGCMCVp()
{
  ...
  if (ngroups > 0) {
    for (int igroup = 0; igroup < ngroups; igroup++) delete[] groupstrings[igroup];
  ...
  if (ngrouptypes > 0) {
    ...
    for (int igroup = 0; igroup < ngrouptypes; igroup++) delete[] grouptypestrings[igroup];
...
void FixGCMCVp::init()
{
  ...
  if (!full_flag) {
    if ((force->kspace) || (force->pair == NULL) || (force->pair->single_enable == 0) ||
        (force->pair_match("hybrid", 0)) || (force->pair_match("eam", 0))) {
      full_flag = true;
      if (comm->me == 0) error->warning(FLERR, "Fix gcmc using full_energy option");
    }
  }
  ...
  if (domain->dimension == 2) error->all(FLERR, "Cannot use fix gcmc in a 2d simulation");
  ...
  if (full_flag) {
    ...
    if (exclusion_group == -1) error->all(FLERR, "Could not find fix gcmc exclusion group ID");
  ...
  if (mode == MOLECULE) {
    ...
    if (molecule_group == -1) error->all(FLERR, "Could not find fix gcmc rotation group ID");
  ...
  if (mode == MOLECULE) {
    ...
    gas_mass = atom->mass[ngcmc_type];

  if (gas_mass <= 0.0) error->all(FLERR, "Illegal fix gcmc gas mass <= 0");
  ...
  if (atom->firstgroup >= 0) {
    ...
    if (flagall) error->all(FLERR, "Cannot do GCMC on atoms in atom_modify first group");
  }
  ...
  double lambda =
      sqrt(force->hplanck * force->hplanck /
           (2.0 * MY_PI * gas_mass * force->mvv2e * force->boltz * reservoir_temperature));
  ...
  if (comm->me == 0) {                                     // Matias
    ...
    printf("pressure_flag equals %i\n", pressure_flag);    // Matias
    printf("pairflag equals %i\n", pairflag);              // Matias
  }                                                        // Matias

  imagezero = ((imageint) IMGMAX << IMG2BITS) | ((imageint) IMGMAX << IMGBITS) | IMGMAX;
  ...
  for (int igroup = 0; igroup < ngroups; igroup++) {
    ...
    if (jgroup == -1) error->all(FLERR, "Could not find specified fix gcmc group ID");
  ...
  if (full_flag) {
    ...
    if (mode == MOLECULE) {
      for (int i = 0; i < ncycles; i++) {
        int random_int_fraction = static_cast<int>(random_equal->uniform() * ncycles) + 1;
...
void FixGCMCVp::attempt_atomic_translation()
{
  ...
  if (i >= 0) {
    ...
    if (random_unequal->uniform() < exp(beta * (energy_before - energy_after))) {
      x[i][0] = coord[0];
      x[i][1] = coord[1];
      x[i][2] = coord[2];
      success = 1;
...
void FixGCMCVp::attempt_atomic_insertion()
{
  ...
  if (triclinic == 0) {
    domain->remap(coord);
    if (!domain->inside(coord)) error->one(FLERR, "Fix gcmc put atom outside box");
  ...
  if (proc_flag) {
    ...
    int nall =
        atom->nlocal + atom->nghost;    // from original 10Feb12 lammps version; added by Jibao
    ...
    if (!pairflag) {                                           // from Matias; added by Jibao
      ...
      insertion_energy =
          pairsw->Stw_GCMC(nall, ngcmc_type, 1, coord);    // from Matias; added by Jibao
    }
    energy_all = insertion_energy;    // from Matias; added by Jibao
    proc_id = comm->me;               // from Matias; added by Jibao

    if (random_unequal->uniform() < zz * volume * exp(-beta * insertion_energy) / (ngas + 1)) {
      ...
      for (int igroup = 0; igroup < ngrouptypes; igroup++) {
        if (ngcmc_type == grouptypes[igroup]) atom->mask[m] |= grouptypebits[igroup];
      }
...
void FixGCMCVp::attempt_molecule_translation()
{
  ...
  if (random_equal->uniform() < exp(beta * (energy_before_sum - energy_after_sum))) {
    for (int i = 0; i < nlocal; i++) {
      if (atom->molecule[i] == translation_molecule) {
...
void FixGCMCVp::attempt_molecule_rotation()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & molecule_group_bit) {
      ...
      domain->remap(xtmp);
      if (!domain->inside(xtmp)) error->one(FLERR, "Fix gcmc put atom outside box");
...
void FixGCMCVp::attempt_molecule_insertion()
{
  ...
  for (int i = 0; i < natoms_per_molecule; i++) {
    ...
    procflag[i] = false;
    if (triclinic == 0) {
      if (xtmp[0] >= sublo[0] && xtmp[0] < subhi[0] && xtmp[1] >= sublo[1] && xtmp[1] < subhi[1] &&
  ...
  if (random_equal->uniform() < zz * volume * natoms_per_molecule *
    ...
    maxmol_all++;
    if (maxmol_all >= MAXTAGINT) error->all(FLERR, "Fix gcmc ran out of available molecule IDs");
    ...
    if (shakeflag) fixshake->set_molecule(nlocalprev, maxtag_all, imol, com_coord, vnew, quat);
...
void FixGCMCVp::attempt_atomic_deletion_full()
{
  ...
  int success = 0;    // added by Jibao

  ndeletion_attempts += 1.0;

  if (ngas == 0) return;

  double deletion_energy = 0.0;            // added by Jibao
  const int i = pick_random_gas_atom();    // added by Jibao

  //double energy_before = energy_stored; // commented out by Jibao
  double energy_before = 0.0;    // added by Jibao
  //energy_before = energy_stored;//debug; Jibao

  if (!pairflag) {                    // added by Jibao
    energy_before = energy_stored;    // added by Jibao
  } else {
    if (i >= 0) {
      pair = force->pair;    //force obtejo que tiene pair           // from Matias, added by Jibao
      deletion_energy =
          pairsw->Stw_GCMC(i, ngcmc_type, 1, atom->x[i]);    // from Matias, added by Jibao
                                                             /*
             if (comm->me == 0) {
  ...
  //const int i = pick_random_gas_atom();   // commented out by Jibao
  ...
  //double energy_after = energy_full();    // commented out by Jibao
  ...
  if (!pairflag) {
    ...
    MPI_Barrier(world);    // from Matias; added by Jibao
    MPI_Allreduce(&success, &success_all, 1, MPI_INT, MPI_MAX, world);
    MPI_Allreduce(&proc_id, &proc_end, 1, MPI_INT, MPI_MAX,
                  world);                                      // from Matias; added by Jibao
    MPI_Bcast(&energy_all, 1, MPI_DOUBLE, proc_end, world);    // from Matias; added by Jibao
    ...
    if (success_all) {
      ...
      atom->nghost =
          0;    // I don't know why to set nghost to zero; need to check it!!!!!!! added and commentted by Jibao
      comm->borders();
      ndeletion_successes += 1.0;
    } else {
      if (i >= 0) {
...
void FixGCMCVp::attempt_molecule_translation_full()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (atom->molecule[i] == translation_molecule) {
      ...
      x[i][1] += com_displace[1];
      x[i][2] += com_displace[2];
      if (!domain->inside_nonperiodic(x[i])) error->one(FLERR, "Fix gcmc put atom outside box");
    }
  }
...
void FixGCMCVp::attempt_molecule_rotation_full()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & molecule_group_bit) {
      ...
      image[i] = imagezero;
      domain->remap(x[i], image[i]);
      if (!domain->inside(x[i])) error->one(FLERR, "Fix gcmc put atom outside box");
      n++;
    }
  }
...
double
FixGCMCVp::molecule_energy(tagint gas_molecule_id)    // TODO Look into this not being in GCMC
{
...
tagint FixGCMCVp::pick_random_gas_molecule()
{
  ...
  MPI_Allreduce(&gas_molecule_id, &gas_molecule_id_all, 1, MPI_LMP_TAGINT, MPI_MAX, world);
...
void FixGCMCVp::toggle_intramolecular(int i)
{
  if (atom->avec->bonds_allow)
    for (int m = 0; m < atom->num_bond[i]; m++) atom->bond_type[i][m] = -atom->bond_type[i][m];

  if (atom->avec->angles_allow)
    for (int m = 0; m < atom->num_angle[i]; m++) atom->angle_type[i][m] = -atom->angle_type[i][m];
...
void FixGCMCVp::update_gas_atoms_list()
{
  ...
  if (nlocal > gcmc_nmax) {
    ...
    gcmc_nmax = atom->nmax;
    local_gas_list = (int *) memory->smalloc(gcmc_nmax * sizeof(int), "GCMC:local_gas_list");
  }
  ...
  if (regionflag) {
    ...
    if (mode == MOLECULE) {
      ...
      for (int i = 0; i < nlocal; i++) {
        if (mask[i] & groupbit) {
          if (domain->regions[iregion]->match(comx[molecule[i]], comy[molecule[i]],
                                              comz[molecule[i]]) == 1) {
